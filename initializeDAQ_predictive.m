function vr = initializeDAQ_predictive(vr)
%Added JPB BAR, Nidaq data session1
vr.moveSession = daq.createSession('ni');
vr.waterSession = daq.createSession('ni');
vr.moveSession.Rate = 2000; % sampling rate of the DAQ
vr.moveSession.DurationInSeconds = .016; % time over which to sample
